...
export default function EventDetails() {
  ...
  const handleBooking = async () => {
    ...
    try {
      ...
      console.error(err);
      const rawError = String(err.response?.data?.message || err.response?.data || "").toLowerCase();
  ...
  return (
    <div className="event-details-container">
      ...
      <p>
        <strong>Available Seats:</strong> <span>{isSoldOut ? "Sold Out" : event.availableSeats}</span>
      </p>
      ...
      {!isSoldOut && (
        <div className="ticket-row">
          ...
          <div className="ticket-counter">
            <button type="button" onClick={() => setTickets((prev) => (prev > 1 ? prev - 1 : 1))} className="counter-btn">
              -
            </button>
            ...
            <button
              type="button"
              onClick={() => setTickets((prev) => (prev < event.availableSeats ? prev + 1 : prev))}
      ...
      >
        {isSoldOut ? "Sold Out" : loading ? "Processing..." : "Book Ticket"}
      </button>

      {message && <p className="booking-message">{message}</p>}

      {showPayment && !isSoldOut && (
        <PaymentModal amount={event.price * tickets} onConfirm={handleBooking} onCancel={() => setShowPayment(false)} isSubmitting={loading} />
      )}
    </div>
  );
}
